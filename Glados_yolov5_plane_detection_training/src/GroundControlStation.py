...
#GUI Parameters
otonom_kalkış_cmd , otonom_iniş_cmd , Manual_cmd , kamikaze_cmd , otonom_it_dalaşı_cmd ,loiter_cmd , guided_cmd , rtl_cmd= False
sel = 0
start = 0
start2 = 0
in_waiting = False
...
class Categorize():
    ...
    def create_message(self):
        global incoming_request ,incoming_wait_ready_request,incoming_attitude        
        ...
        if incoming_request == "Fight":
            incoming_attitude = " "
        tx_data = {"incoming_request":incoming_request,"incoming_attitude":incoming_attitude}
        
    def create_decision(self,point_list,in_range_list,characteristics_list):
        print(point_list,in_range_list,characteristics_list)
        if in_range_list[0] != 0 or in_range_list[1] != 0 or in_range_list[2] != 0 :
            escape_cmd = True
        else:
            escape_cmd = False
    ...
    def flight_controller(self):
        ...
        if enemy_list != 0 and type(enemy_list) == list:
            ...
            if prediction_list != 0:
                point_list = self.create_route(prediction_list)
                in_range_list , characteristics_list = self.create_escape()
                self.create_decision(point_list,in_range_list,characteristics_list)
                self.create_message()
                        ...
                        for key, mask in events:
                            ...
                            try:
                                ...
                                if enemy_list != 0 :
                                    ...
                                    client_socket_uav.send(enemy_list_copy)  # send message
