...
def read_incoming_packet(client, packet):
    ...
    packet_list = [packet.hex()[i:i+2] for i in range(8, len(packet.hex())-8, 2)]
    tmp_storage = []
    count=0
    comma_count = 0
    calc_count = 0
    ...
    for i in packet_list: 
        if i == '2c' or i == '24':
            step_count.append(calc_count)
            packet_list.pop(count)
    ...
    count =0
    tmp_storage =[int(i) for i in packet_list]
    #tmp_storage =[int(i) for i in packet_list]
    for temp_str in tmp_storage:
        tmp_storage[count]=temp_str-30
        count=count+1

    for i in range(comma_count):
            if step_count[i] == 1:
                packet_list[index_count] = str(tmp_storage[step_count[i]-1 + array_count])
                array_count = array_count + 1 
            elif step_count[i] == 2:
                first_step=tmp_storage[step_count[i]-2+array_count]*10
                temp_second_step=first_step + tmp_storage[step_count[i]-1 + array_count]
                packet_list[index_count] = str(temp_second_step)
                array_count = array_count + 2
            elif step_count[i] == 3:
                first_step=tmp_storage[step_count[i]-3 + array_count]*100
                second_step =tmp_storage[step_count[i]-2 + array_count]*10
                temp_third_step=first_step + second_step + tmp_storage[step_count[i]-1 + array_count]
                packet_list[index_count] = str(temp_third_step)
                array_count = array_count + 3
            index_count=index_count +1
    while len(packet_list)>index_count:
        packet_list.pop(index_count)
        if len(packet_list)==index_count:
            break
    ...
    if (protocol_name == 'login'):
        ...
        if (packet_list[0] == '06' or packet_list[0] == '6'): 
            print('[', addresses[client]['address'][0], ']', 'STATUS : Battery =',packet_list[2], '; Sw v. =',packet_list[3], '; Status upload interval =',packet_list[4])
        elif (packet_list[0] == '07' or packet_list[0] == '7'): 
            print('[', addresses[client]['address'][0], ']', 'STATUS : Battery =', packet_list[2], '; Sw v. =',packet_list[3], '; Status upload interval =',packet_list[4], '; Signal strength =', packet_list[5])
...
GMAPS_API_KEY = 'AIzaSyC3JNZjYc23yquxRX3aRn7TE_vDmOUF6Ys'
